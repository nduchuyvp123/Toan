...
\begin{definition}[Đồ thị phân chia]
    \textit{Chia nhỏ} một cạnh là phép toán trong thay thế một cạnh bằng 2 cạnh được nối với một đỉnh mới, đỉnh mới được thêm vào đồ thị ban đầu.
...
\begin{definition}
    ...
    \textit{Bậc liên thông} của $G$, kí hiệu $\kappa(G)$, là minimum size của \textit{tập cắt} $V'$. Đồ thị $G$ là k-connected nếu $k \leq \kappa(G)$.
\end{definition}
\begin{center}
    \begin{tikzpicture}
        ...
        \node at (2,-1,0) {Đồ thị 2-connected};
    \end{tikzpicture}
    \hspace{2cm}
    \begin{tikzpicture}
        \draw[black, thick] (0,0) -- (0,4);
        ...
        \node at (2,-1,0) {Không phải 2-connected};
    \end{tikzpicture}
\end{center}