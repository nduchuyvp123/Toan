...
int main(int argc, char const *argv[])
{
    ...
    while (count < 50000)
    {
        ...
        VectorXd b = (x0 - y) - beta_n * Fx(x0);
        double con_x = scalar(b, x0 - gamma_ * lamda_n * beta_n * Fx(y)) - scalar(y, b);
        VectorXd x1 = x0 - gamma_ * lamda_n * beta_n * Fx(y);
        if (con_x > 0)
        {
            x1 = x1 - (con_x / (b.norm() * b.norm())) * b;
        }
        x0 = x1;
        m = x0.norm();
